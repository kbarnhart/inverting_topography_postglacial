...
initial_dem_filepath = ['..', '..', 'auxillary_inputs', 'dems', 'validation', 'modern', 'vmodern24fr3.txt']
chi_mask_filepath = ['..', '..', 'auxillary_inputs', 'chi_mask', 'validation' , 'chi_mask.txt']
rock_till_filepath = ['..', '..', 'auxillary_inputs', 'rock_till', 'validation' , 'valbdx24bnd2.txt']
...
driver_template_filepath = os.path.abspath(os.path.join(*(input_template_folderpath+['validation_best_param_model_driver_template.txt'])))
...
#%%
category_file = os.path.join(os.path.abspath(os.sep), *['work', 'WVDP_EWG_STUDY3', 'study3py','auxillary_inputs', 'chi_elev_categories', 'validation.chi_elev_cat.20.txt'])
category_weight_file = os.path.join(os.path.abspath(os.sep), *['work', 'WVDP_EWG_STUDY3', 'study3py','auxillary_inputs', 'weights', 'validation.chi_elev_weight.20.txt'])
...
for model_name in model_dictionary.keys():
    ...
    if type(mid_used[model_name]) == str:
        ...
        lines = ['# Inputs for Model: ' + model_name,
                 ...
                 'category_weight_file: ' + category_weight_file]
        ...
        input_template_filepath = os.path.abspath(os.path.join(*(input_template_folderpath+['validation_best_param_inputs_template_'+model_name+'.txt'])))
        ...
        # Write model driver
        model_driver_filepath = os.path.abspath(os.path.join(*(driver_folderpath+['validation_best_param_'+model_name+'_driver.py'])))
