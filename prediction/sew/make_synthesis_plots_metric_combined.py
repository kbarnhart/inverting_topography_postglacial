...
out_folder = "synthesis_plots_meters_combined"
...
plot_text = {"only842": "Only BasicChRtTh (842)", "all800s": "All nine Rt variants"}
...
h=30 
dx = 24
times = np.arange(0, 10100, 100)

for time in times:
    letters = list("abcdefghijklmnopqrstuvwxyz")

    fig = plt.figure(figsize=(3.5, 7), dpi=300)  # convert mm to inches
    ...
    for col, set_key in enumerate(set_keys):
        ...
        if set_key == "all800s":
            ...
            sm = plt.cm.ScalarMappable(
                cmap="gist_ncar_r", norm=plt.Normalize(vmin=0, vmax=std_lim * ft2m)
            )
            cbar = fig.colorbar(sm, cax=ax, orientation="horizontal")
        ...
        imshow_grid(
            mg,
            ds.std_total_topo.values.flatten() * ft2m,
            color_for_closed=None,
            allow_colorbar=False,
            vmin=0,
            vmax=std_lim,
    ...
    delta = 1/7.
    ax = fig.add_subplot(spec[1:, 2:])
    ax.plot([0, 0.5, 0.5, 1], [5, 5, 4.45, 4.45], linewidth=1, linestyle="dashed", color="k")
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 6)
    ax.patch.set_alpha(0. )
    plt.axis("off")
    
    plt.savefig(basic_fig_name + "_combined.png")
    plt.savefig(basic_fig_name + "_combined.pdf")
