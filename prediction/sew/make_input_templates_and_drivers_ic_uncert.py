...
driver_template_filepath = os.path.abspath(os.path.join(*(input_template_folderpath+['sew_prediction_ic_uncert_model_driver_template.txt'])))
...
for model_name in model_dictionary.keys():
    ...
    if type(mid_used[model_name]) == str:
        ...
        # Write model driver
        model_driver_filepath = os.path.abspath(os.path.join(*(driver_folderpath+['sew_prediction_ic_uncert_'+model_name+'_driver.py'])))
