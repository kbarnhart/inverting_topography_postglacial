...
work_directory_folderpath = ['work', 'WVDP_EWG_STUDY3', 'results','calibration', 'sew', 'Evaluate_Best_Parameters'] # path to scratch where model will be run.
...
for model_name in  sorted(list(model_dictionary.keys())):
    ...
    if os.path.exists(table_file):
        ...
        # Get input template
        input_template_filepath = os.path.abspath(os.path.join(dir_path, *(input_template_folderpath+['sew_best_param_inputs_template_'+model_name+'.txt'])))
        with open(input_template_filepath,'r') as itfp:
            input_template_lines = itfp.readlines()
        
        # get model driver name
        model_driver_name = os.path.abspath(os.path.join(dir_path, *(models_driver_folderpath+['sew_best_param_'+model_name+'_driver.py'])))
